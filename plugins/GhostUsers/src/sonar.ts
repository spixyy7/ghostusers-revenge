...
function mentions(value: any, id: string, depth = 0, budget = { n: 300 }): boolean {
    if (budget.n-- <= 0 || value == null || depth > 3) return false;
...
export function startSonar(patches: (() => void)[]) {
    ...
    for (const s of stores) {
        ...
        ].filter(k => k.startsWith("get") && typeof (s as any)[k] === "function");
